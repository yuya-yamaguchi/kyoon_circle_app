...
RSpec.describe 'Events', type: :request do
  ...
  describe '#update' do
    context '管理者権限がある場合' do
      it '[HTTP:204] 対象レコードが更新されること' do
        put "/api/events/#{events[0].id}", params: { event: event_params, user_id: admin_user.id },
    ...
    context '管理者権限がない場合' do
      it '[HTTP:403] 対象レコードが更新されないこと' do
        put "/api/events/#{events[0].id}", params: { event: event_params, user_id: user.id },
  ...
  describe '#create' do
    subject do
      post '/api/events', params: { event: event_params, user_id: admin_user.id },
                          headers: { 'Authorization' => admin_user.token }
    end
    context '管理者権限がある場合' do
