class StudioReserve < ApplicationRecord
  ...
  def validate_reserve_duplicate
    studio_reserves = StudioReserve.where(studio_id: self.studio_id)
                                   .where(date: self.date)
    is_reserved = false
    # 対象の予約日内で時間の重複がないかチェック(UTC時刻だと不整合のケースがあるためSQLで比較せず抽出後JSTで比較)
    studio_reserves.each do |reserve|
      if reserve.start_time.to_s(:time) >= self.start_time.to_s(:time)
        if reserve.start_time.to_s(:time) < self.end_time.to_s(:time)
          is_reserved = true
          break
        end
      else
  ...
  def validate_before_today
    ...
    if self.date < now.to_date
      is_cant_cancel = true
    elsif self.date == now.to_date
      is_cant_cancel = true if self.start_time.to_s(:time) <= now.to_s(:time)
    end
    if is_cant_cancel
